...
public class DropDownTests extends TestBase {
    ...
    */
    ...
    public void verifyIfTheDDsAreWorkingAsExpected() {
        ...
        Assert.assertEquals("The number is wrong", NUMBER_OF_VALUES_FROM_OLD_SELECT_MENU, oldSelectMenuDD.getOptions().size());
        ...
        Assert.assertEquals("The size of list is wrong", 2, selectedColors.size());
        ...
        Assert.assertEquals("The size of list is wrong", 3, listOfCarsExceptAudi.size());
        System.out.println("Cars list size " + listOfCarsExceptAudi.size());

        log.info("Assert false that the new list does not contain Audi.");
        Assert.assertFalse("The list contains Audi", listOfCarsExceptAudi.contains(CAR_TYPE_AUDI));
